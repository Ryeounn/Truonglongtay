...
function Loginforteacher() {
    ...
    const handleLogin = () => {
        ...
        if (captchaValue === null) {
            Swal.fire({
                title: 'Thất bại',
                text: 'Vui lòng hoàn thành CAPCHA',
                icon: 'error',
        ...
        axios.post('http://localhost:8888/account/loginteacher', {
            studentCode,
            password
        }).then(response => {
            ...
            if (status == "ROLETEACHER") {
                ...
                navigate('/teachers');
            } else if (status == "ROLEACCOUNTANCY") {
                sessionStorage.setItem('userId', userId);
                sessionStorage.setItem('role', 'accountancy');
                sessionStorage.setItem('teachername', teachername);
                navigate('/homepage');
            }
            else {
