...
const Uniformmanage1 = () => {
    ...
    const handleSetting = (id) => {
        axios.post("http://localhost:8888/api/productbyid", { id: id })
            .then(response => {
    ...
    const productData = {
        ...
        try {
            const response = await axios.post('http://localhost:8888/api/edit', productData, {
                headers: {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.post(`http://localhost:8888/api/delete`, { id })
                    .then(response => {
                                        ...
                                        {currentProducts.map((product, index) => {
                                            ...
                                            return (
                                                <tr className="border-b-[1px] border-[#ccc] border-solid p-[20px_0]" key={product.id}>
                                                    ...
                                                    <td><img src={product.imageuniform} className="w-[60px] h-[80px] rounded-10 m-[20px_0]" alt={product.uniform} /></td>
    ...
    const handleSubmit = async (e) => {
        ...
        if (image) {
            try {
                ...
                await axios.post('http://localhost:8888/api/add', productData);
                setName('');
                setPrice('');
                setSize('');
                setQuantity('');
